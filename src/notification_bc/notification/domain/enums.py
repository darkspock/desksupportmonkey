from enum import Enum


class EventType(str, Enum):
    REQUEST_CREATED = "request.created"
    REQUEST_STATUS_CHANGED = "request.status_changed"
    REQUEST_ASSIGNED = "request.assigned"
    REQUEST_PRIORITY_CHANGED = "request.priority_changed"
    REQUEST_COMMENT_ADDED = "request.comment_added"
    REQUEST_NOTE_ADDED = "request.note_added"
    REQUEST_APPROVAL_NEEDED = "request.approval_needed"
    REQUEST_APPROVED = "request.approved"
    REPORT_READY = "report.ready"
    PO_SUBMITTED = "po.submitted"
    PO_APPROVED = "po.approved"
    PO_CANCELLED = "po.cancelled"
    BUDGET_THRESHOLD_REACHED = "budget.threshold_reached"
    PO_RECEIVED = "po.received"
    APPOINTMENT_CREATED = "appointment.created"
    APPOINTMENT_CONFIRMED = "appointment.confirmed"
    APPOINTMENT_CANCELLED = "appointment.cancelled"
    APPOINTMENT_RESCHEDULED = "appointment.rescheduled"
    APPOINTMENT_COMPLETED = "appointment.completed"
    APPOINTMENT_REMINDER = "appointment.reminder"
    SHIPMENT_CREATED = "shipment.created"
    SHIPMENT_DISPATCHED = "shipment.dispatched"
    SHIPMENT_DELIVERED = "shipment.delivered"
    SHIPMENT_FAILED = "shipment.failed"
    SHIPMENT_CANCELLED = "shipment.cancelled"
    MAINTENANCE_SCHEDULED = "maintenance.scheduled"
    MAINTENANCE_DUE = "maintenance.due"
    MAINTENANCE_OVERDUE = "maintenance.overdue"
    MAINTENANCE_COMPLETED = "maintenance.completed"
    MAINTENANCE_CANCELLED = "maintenance.cancelled"
    INCIDENT_CREATED = "incident.created"
    INCIDENT_STATUS_CHANGED = "incident.status_changed"
    INCIDENT_SEVERITY_CHANGED = "incident.severity_changed"
    INCIDENT_ASSIGNED = "incident.assigned"
    INCIDENT_EMPLOYEE_REPORTED = "incident.employee_reported"
    INCIDENT_DEADLINE_WARNING = "incident.deadline_warning"
    INCIDENT_DEADLINE_URGENT = "incident.deadline_urgent"
    INCIDENT_DEADLINE_PASSED = "incident.deadline_passed"
    RISK_REVIEW_OVERDUE = "risk.review_overdue"
    AUDIT_EXPORT_READY = "audit.export_ready"
    GDPR_DATA_EXPORT_READY = "gdpr.data_export_ready"
    GDPR_ANONYMIZATION_COMPLETED = "gdpr.anonymization_completed"
    SLA_WARNING = "sla.warning"
    SLA_RESPONSE_BREACHED = "sla.response_breached"
    SLA_RESOLUTION_BREACHED = "sla.resolution_breached"
    COMPLIANCE_REPORT_READY = "compliance.report_ready"
    CONTRACT_RENEWAL_REMINDER = "vendor.contract_renewal_reminder"
    CONCENTRATION_RISK_ALERT = "vendor.concentration_risk_alert"
    VENDOR_ASSESSMENT_OVERDUE = "vendor.assessment_overdue"
    VULNERABILITY_CRITICAL_REGISTERED = "vulnerability.critical_registered"
    VULNERABILITY_REMEDIATION_OVERDUE = "vulnerability.remediation_overdue"
    CHANGE_APPROVED = "change.approved"
    CHANGE_REJECTED = "change.rejected"
