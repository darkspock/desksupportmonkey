...
class TestVerifyMagicLinkWithMembership:
    """Tests for verify with scoped auth (membership resolution)."""
    ...
    def setup_method(self):
        ...
        self.company_lookup = MagicMock()
        self.jwt_service = MagicMock()
        self.membership_auth = MagicMock()
        ...
        self.handler = VerifyMagicLinkService(
            ...
            company_lookup=self.company_lookup,
            jwt_service=self.jwt_service,
            membership_auth=self.membership_auth,
            company_repo=self.company_repo,
        )

    def test_scoped_verify_resolves_membership(self):
        ml = MagicLink.create(email="user@company.com", company_id="comp123")
        user = _make_user()
        company = MagicMock()
        ...
        self.membership_auth.resolve_membership.return_value = user
        self.jwt_service.create_token.return_value = "jwt-scoped"

        result = self.handler.handle(VerifyMagicLinkRequest(token=ml.token))

        assert result == "jwt-scoped"
        self.membership_auth.resolve_membership.assert_called_once_with(
            user, "comp123", "domain"
        )

    def test_unscoped_verify_skips_membership_resolution(self):
        ml = _make_valid_magic_link()  # company_id=None
        user = _make_user()

        self.magic_link_repo.find_by_token.return_value = ml
        self.company_lookup.find_company_by_email_domain.return_value = ("comp123", True)
        self.user_repo.find_by_email.return_value = user
        self.jwt_service.create_token.return_value = "jwt-unscoped"

        result = self.handler.handle(VerifyMagicLinkRequest(token=ml.token))

        assert result == "jwt-unscoped"
        self.membership_auth.resolve_membership.assert_not_called()

    def test_scoped_verify_without_membership_service_skips(self):
        """When membership_auth is None, skip membership resolution."""
        handler = VerifyMagicLinkService(
            magic_link_repo=self.magic_link_repo,
            user_repo=self.user_repo,
            company_lookup=self.company_lookup,
            jwt_service=self.jwt_service,
            membership_auth=None,
            company_repo=None,
        )
        ml = MagicLink.create(email="user@company.com", company_id="comp123")
        user = _make_user()
        ...
        self.company_lookup.find_company_by_email_domain.return_value = ("comp123", True)
        self.user_repo.find_by_email.return_value = user
        self.jwt_service.create_token.return_value = "jwt-no-membership"

        result = handler.handle(VerifyMagicLinkRequest(token=ml.token))

        assert result == "jwt-no-membership"

    def test_scoped_verify_company_not_found_skips_membership(self):
        ml = MagicLink.create(email="user@company.com", company_id="comp123")
        user = _make_user()
        ...
        self.company_lookup.find_company_by_email_domain.return_value = ("comp123", True)
        self.user_repo.find_by_email.return_value = user
        self.company_repo.find_by_id.return_value = None
        self.jwt_service.create_token.return_value = "jwt-no-company"

        result = self.handler.handle(VerifyMagicLinkRequest(token=ml.token))

        assert result == "jwt-no-company"
        self.membership_auth.resolve_membership.assert_not_called()
