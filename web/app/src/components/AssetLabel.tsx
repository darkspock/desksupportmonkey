  ...
  return (
    <Card>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-semibold text-gray-900">{t('page.asset_detail.label_title')}</h3>
        <button
          type="button"
          onClick={handlePrint}
          className="inline-flex items-center gap-1.5 rounded border px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-50"
        >
      ...
      <div className="flex flex-wrap items-start gap-8">
        <div className="flex flex-col items-center gap-1">
          <p className="text-[10px] uppercase tracking-wider text-gray-400">{t('page.asset_detail.qr_code')}</p>
          <QRCodeSVG id="asset-qr-code" value={assetUrl} size={120} level="M" />
          <p className="mt-1 max-w-[140px] truncate text-[10px] text-gray-400" title={assetUrl}>
            {assetUrl.replace(/^https?:\/\//, '')}
          </p>
        </div>

        <div className="flex flex-col items-center gap-1">
          <p className="text-[10px] uppercase tracking-wider text-gray-400">{t('page.asset_detail.barcode')}</p>
          <svg ref={barcodeRef} />
        </div>
      </div>
    </Card>
