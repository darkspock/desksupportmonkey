...
const FRAMEWORK_OPTIONS = ['NIS2', 'DORA', 'ISO 27001', 'GDPR'];
...
export default function ComplianceDashboardPage() {
  ...
  const [framework, setFramework] = useState<string>(searchParams.get('framework') || '');
  ...
  const hasControls = dashboard.total_controls > 0;
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Empty State */}
      {!hasControls && (
        <div className="rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800/50 px-6 py-12 text-center">
          <svg viewBox="0 0 24 24" className="h-12 w-12 mx-auto text-gray-400 dark:text-gray-500 mb-4" fill="none" stroke="currentColor" strokeWidth="1.5">
            <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z" />
          </svg>
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">{t('compliance.empty_title')}</h3>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">{t('compliance.empty_description')}</p>
          <Link
      ...
      {/* Framework Filter + Summary — only when there are controls */}
      {hasControls && (
        <>
