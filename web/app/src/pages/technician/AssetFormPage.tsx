...
export default function AssetFormPage() {
  ...
  const mutation = useMutation({
    ...
    onError: (err: unknown) => {
      setError((err as { response?: { data?: { detail?: string } } })?.response?.data?.detail || 'Failed to create asset');
    },
  });
  ...
  return (
    <div className="max-w-xl">
      <h2 className="text-xl font-bold text-gray-900 mb-4">New Asset</h2>
      <Card>
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(); }} className="space-y-4">
          ...
          {['brand', 'model', 'serial_number'].map((field) => (
            <div key={field}>
              <label className="block text-sm font-medium text-gray-700 mb-1">{field.replace('_', ' ')}</label>
